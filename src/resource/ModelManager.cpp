...
void ModelManager::LoadModel(HashedString modelName,
                             const std::string& modelPath, bool backfaceCull) {
  ...
  LOG_INFO("Mats: %i, meshes: %i, textures: %i ", scene->mNumMaterials,
...
void ModelManager::ProcessMesh(aiMesh* aiMesh, const aiScene* scene,
  ...
  for (uint32_t i = 0; i < aiMesh->mNumVertices; i++) {
    ...
    if (aiMesh->HasPositions()) {
      v.position = glm::vec3(aiMesh->mVertices[i].x, aiMesh->mVertices[i].y,
                             aiMesh->mVertices[i].z);
    }
    if (aiMesh->HasNormals()) {
      v.normal = glm::vec3(aiMesh->mNormals[i].x, aiMesh->mNormals[i].y,
                           aiMesh->mNormals[i].z);
    }
    if (aiMesh->HasTextureCoords(0)) {
      v.texCoords = glm::vec2(aiMesh->mTextureCoords[0][i].x,
  ...
  auto mesh_name = HashedString(
      (std::string(m_path) + std::to_string(m_group->m_objects.size()))
          .c_str());
  ...
  if (aiMesh->mMaterialIndex >= 0) {
    ...
    auto mat_name = HashedString(
        (std::string(m_path) + std::to_string(aiMesh->mMaterialIndex)).c_str());
...
Material* ModelManager::LoadMaterial(aiMaterial* aiMat, HashedString matName) {
  ...
  auto diffuse_maps = LoadMaterialTextures(aiMat, aiTextureType_DIFFUSE,
                                           MatTextureType::Diffuse);
  textures.insert(textures.end(), diffuse_maps.begin(), diffuse_maps.end());
  auto specular_maps = LoadMaterialTextures(aiMat, aiTextureType_SPECULAR,
                                            MatTextureType::Specular);
  textures.insert(textures.end(), specular_maps.begin(), specular_maps.end());

  MaterialManager::AddMaterial(matName, textures, "blinnPhong",
...
std::vector<TexturePair> ModelManager::LoadMaterialTextures(
  ...
  for (uint32_t i = 0; i < aiMaterial->GetTextureCount(aiType); i++) {
    ...
    auto texture_name =
        HashedString((std::string(m_path) + texture_filename.data).c_str());
    //    std::cout << m_name << " " << textureFilename.data << std::endl;
    Texture* texture = TextureManager::GetTexture(texture_name);
    if (texture == nullptr) {
      texture = TextureManager::AddTexture(
          texture_name, std::string(m_directory) + '/' + texture_filename.data,
