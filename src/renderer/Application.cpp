...
void Application::SetupResources() {
  ...
  std::vector<TexturePair> woodContainerTextures = {
      {MatTextureType::Diffuse,
       TextureManager::GetTexture("woodContainerDiffuse")},
      {MatTextureType::Specular,
       TextureManager::GetTexture("woodContainerDiffuse")}};
  MaterialManager::AddMaterial("woodContainer", woodContainerTextures,
                               "blinnPhong");
}
...
void Application::OnImGui() {
  ...
  if (ImGui::CollapsingHeader("Render Settings",
    ...
    if (ImGui::BeginCombo(
            "##RenderModeCombo",
            ("Mode: " + std::string(items[selectedItem])).c_str())) {
    ...
    ImGui::InputTextWithHint("##screenshotFilename", "Screenshot Filename",
                             screenshotBuffer, sizeof(screenshotBuffer));
  }
  ...
  if (m_renderToImGuiViewport) {
    ...
    if (prevDimensions.x != currDimensions.x ||
        prevDimensions.y != currDimensions.y) {
      // update cameras
      float aspectRatio = static_cast<float>(currDimensions.x) /
                          static_cast<float>(currDimensions.y);
...
void Application::OnMousePosMove(double xpos, double ypos) {
  if (!m_renderToImGuiViewport && ImGui::GetIO().WantCaptureMouse)
    return;
  static bool isFirstMouse = true;
...
void Application::OnSceneChange() {
  ...
  m_cameraController.GetActiveCamera()->SetPosition(
      activeScene->defaultCameraPosition);
